...
template <typename AccType>
void gemmBinary_neon_tile2x2(uint64_t * A, uint64_t * BT, AccType * CT, AccType alpha,
uint64_t rowsA, uint64_t depth_words, uint64_t rowsBT) {
  assert(rowsA % 2 == 0);
  assert(rowsBT % 2 == 0);
  ...
  for(uint64_t lbit = 0; lbit < lhsbits; lbit++) {
    ...
    for(uint64_t rbit = 0; rbit < rhsbits; rbit++) {
      ...
      if(lhs->nrows % 2 == 0 && rhs->nrows % 2 == 0 && lhs->wordsPerRow() % 2 == 0) {
        gemmBinary_neon_tile2x2(lhs->bitplaneptr(lbit), rhs->bitplaneptr(rbit), res, alpha, lhs->nrows, lhs->wordsPerRow(), rhs->nrows);
      }
